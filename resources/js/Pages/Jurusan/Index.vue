...
const submitAdd = () => {
    addForm.post(route('jurusan.store'), {
        onSuccess: () => {
            closeAddModal();
            addForm.reset();
        },
...
const openEditModal = (major) => {
    ...
    editForm.id = major.id;
    editForm.nama = major.nama;
    editForm.kode = major.kode;
...
const submitEdit = () => {
    editForm.put(route('jurusan.update', editForm.id), {
        onSuccess: () => {
            closeEditModal();
        },
    });
...
const confirmDelete = () => {
    router.delete(route('jurusan.destroy', selectedMajor.value.id), {
        onSuccess: () => {
            closeDeleteModal();
        },
    });
...
const stats = computed(() => {
    return {
        total: props.majors.total || 0,
    };
});
</script>

<template>
    <Head title="Data Jurusan BK" />
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Kelola data kompetensi keahlian / jurusan sekolah.</p>
                </div>
                <div>
                    <button 
                        @click="openAddModal"
        ...
        <div class="py-8 relative overflow-hidden min-h-screen">
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6 relative z-10">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                    <Card variant="indigo" hoverable class="relative overflow-hidden group">
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="text-xs font-bold text-blue-100/80 uppercase tracking-wider">Total Jurusan</p>
                ...
                <!-- Majors Table Card -->
                <Card variant="glass">
                    <Table :headers="['Kode Jurusan', 'Nama Jurusan', 'Aksi']">
                        <tr 
                            v-for="m in majors.data" 
                            :key="m.id"
                            class="hover:bg-blue-50/20 dark:hover:bg-blue-950/10 transition duration-150"
                        >
                            <!-- Kode -->
                            <td class="px-6 py-4">
                                <span class="text-xs font-black bg-blue-100 text-blue-850 dark:bg-blue-950/40 dark:text-blue-350 px-2.5 py-1 rounded-lg border border-blue-200/50 dark:border-blue-900/50">
                                    {{ m.kode }}
                                </span>
                            </td>
                        ...
                        <!-- Empty State -->
                        <tr v-if="!majors.data || majors.data.length === 0">
                            <td colspan="3" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center gap-3">
                                    ...
                                    <h4 class="font-bold text-slate-800 dark:text-slate-200">Jurusan tidak ditemukan</h4>
                                    <p class="text-xs text-slate-400 max-w-[280px]">Mulai dengan menambahkan data Jurusan ke dalam sistem.</p>
                                </div>
                            </td>
                        </tr>
                    </Table>
                    ...
                    <!-- Pagination -->
                    <div class="mt-6 flex items-center justify-between" v-if="majors.links && majors.links.length > 3">
                        <span class="text-xs text-slate-400 font-semibold">
                            Menampilkan {{ majors.from || 0 }} - {{ majors.to || 0 }} dari {{ majors.total }} jurusan
                        </span>
                        <div class="flex items-center gap-1.5">
                            <button 
                                v-for="link in majors.links" 
                                :key="link.label"
        ...
        <div v-if="showAddModal" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
            <div class="bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-850 max-w-md w-full overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                <div class="px-6 py-4 border-b border-gray-100 dark:border-slate-900 flex justify-between items-center bg-gradient-to-r from-blue-50/50 to-blue-50/50 dark:from-slate-900/50 dark:to-slate-900/50">
                    <h3 class="font-extrabold text-slate-900 dark:text-white text-lg">Tambah Jurusan</h3>
                ...
                <form @submit.prevent="submitAdd" class="p-6 space-y-4">
                    <!-- Kode Jurusan -->
                    <div>
                        <label class="block text-xs font-black text-slate-500 uppercase tracking-wide mb-1">Kode Jurusan <span class="text-red-500">*</span></label>
                        <input v-model="addForm.kode" type="text" placeholder="Contoh: RPL, TKJ, DKV..." required
                            ...
                            :class="addForm.errors.kode ? 'border-red-500' : 'border-gray-200'"
                        />
                        <p class="text-[10px] text-red-500 mt-1" v-if="addForm.errors.kode">{{ addForm.errors.kode }}</p>
                    </div>

                    <!-- Nama Jurusan -->
                    <div>
                        <label class="block text-xs font-black text-slate-500 uppercase tracking-wide mb-1">Nama Jurusan <span class="text-red-500">*</span></label>
                        <input v-model="addForm.nama" type="text" placeholder="Contoh: Rekayasa Perangkat Lunak..." required
                            class="w-full text-sm py-2 px-3 rounded-lg border dark:border-slate-800 bg-transparent focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden dark:text-white placeholder-slate-400"
                    ...
                    <div class="pt-4 border-t border-gray-100 dark:border-slate-900 flex justify-end gap-3">
                        ...
                        >
                            {{ addForm.processing ? 'Menyimpan...' : 'Simpan Jurusan' }}
                        </PrimaryButton>
                    </div>
                </form>
            </div>
        ...
        <div v-if="showEditModal" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
            <div class="bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-850 max-w-md w-full overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                <div class="px-6 py-4 border-b border-gray-100 dark:border-slate-900 flex justify-between items-center bg-gradient-to-r from-blue-50/50 to-sky-50/50 dark:from-slate-900/50 dark:to-slate-900/50">
                    <h3 class="font-extrabold text-slate-900 dark:text-white text-lg">Ubah Jurusan</h3>
                ...
                <form @submit.prevent="submitEdit" class="p-6 space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-black text-slate-500 uppercase tracking-wide mb-1">Kode Jurusan <span class="text-red-500">*</span></label>
                        <input v-model="editForm.kode" type="text" placeholder="Contoh: RPL, TKJ..." required
                            class="w-full text-sm py-2 px-3 rounded-lg border dark:border-slate-800 bg-transparent focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden dark:text-white placeholder-slate-400"
                            :class="editForm.errors.kode ? 'border-red-500' : 'border-gray-200'"
                        />
                        <p class="text-[10px] text-red-500 mt-1" v-if="editForm.errors.kode">{{ editForm.errors.kode }}</p>
                    </div>

                    <!-- Nama Jurusan -->
                    <div>
                        <label class="block text-xs font-black text-slate-500 uppercase tracking-wide mb-1">Nama Jurusan <span class="text-red-500">*</span></label>
                        <input v-model="editForm.nama" type="text" placeholder="Contoh: Rekayasa Perangkat Lunak..." required
                            class="w-full text-sm py-2 px-3 rounded-lg border dark:border-slate-800 bg-transparent focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-hidden dark:text-white placeholder-slate-400"
                    ...
                    <div class="pt-4 border-t border-gray-100 dark:border-slate-900 flex justify-end gap-3">
                        ...
                        >
                            {{ editForm.processing ? 'Menyimpan...' : 'Perbarui Jurusan' }}
                        </PrimaryButton>
                    </div>
                </form>
            </div>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
            <div class="bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-850 max-w-sm w-full p-6 text-center shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                ...
                <h3 class="text-base font-extrabold text-slate-900 dark:text-white mb-2">Hapus Data Jurusan?</h3>
                <p class="text-xs text-slate-400 mb-6 leading-relaxed">
                    Apakah Anda yakin ingin menghapus jurusan <strong>{{ selectedMajor?.nama }} ({{ selectedMajor?.kode }})</strong>? Tindakan ini bersifat permanen dan tidak bisa dibatalkan.
                </p>
